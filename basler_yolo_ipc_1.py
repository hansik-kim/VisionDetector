    ...
    try:
        while camera.IsGrabbing():
            ...
            if grab_result.GrabSucceeded():
                ...
                # ============== 공유 폴더에 저장 ==============
                # 원자적(Atomic) 저장을 위해 임시 파일에 쓰고 이름을 변경합니다.
                # 이렇게 하면 UI 쪽에서 쓰고 있는 파일을 읽다가 깨지는 현상을 방지할 수 있습니다.
                temp_original = SHARE_DIR / "original_tmp.jpg"
                temp_result = SHARE_DIR / "result_tmp.jpg"

                cv2.imwrite(str(temp_original), frame)
                cv2.imwrite(str(temp_result), annotated)

                try:
                    os.replace(temp_original, ORIGINAL_IMAGE_PATH)
                    os.replace(temp_result, RESULT_IMAGE_PATH)
                except PermissionError:
                    # UI 프로그램 등이 파일을 잡고 있어서 교체가 실패할 경우 이번 프레임은 스킵
                    pass
